...
class NewsController{
    ...
    async show(req,res){
        const nData = await News.findOne({ _id: req.params.id }).populate('postedBy', 'name email').populate('categoryId', 'category_name');
        if(nData.image){
            nData.image = `${process.env.PUBLIC_URL}/news/${nData.image}`;
        }else{
            nData.image = `${process.env.PUBLIC_URL}/icons/notfound.png`;
        }
        return res.status(200).json({ status: true, news: nData });
